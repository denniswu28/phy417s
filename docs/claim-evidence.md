...
| `GS-DET-001` | Two runs of the same configuration and seed produce **byte-identical** artifacts within the pinned environment of `constraints-ci.txt`. | `tests/test_demo.py::test_two_runs_in_separate_directories_are_byte_identical`; the double-run step in `.github/workflows/ci.yml`. | `reproduced`, scoped to the pinned environment on Python 3.12. No claim is made for any other environment. |
